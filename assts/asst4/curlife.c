...
* First version written 1983 by TEK in Applesoft BASIC.
* Written Nov, 1998 by TEK in GNU C and ncurses. 
* Modified to create default startup file Dec 1999 by TEK.  
* Ported to gcc/Solaris Dec, 2000 by TEK.
* to compile under Linux: 
* 	cc -o curlife curlife.c -lncurses
* to compile at rohan.sdsu.edu (Solaris):
* 	gcc -o curlife curlife.c -lcurses
Note to the student: this program seizes control of the
system kill interrupt, so the user can't arbitrarily kill
the run.  To understand the reason for this, 
read the die() function code, 
and enter "man signal" at the Unix prompt.
Once you understand this, please use it with compassion,
and invisibly, as here.  Nobody likes a control freak.
------------------------------------------------------------*/
#include <signal.h>
#include <curses.h>
#include <stdio.h>
...
#define STARTC 0
#undef FAST 
int screen[ROWS+1][COLS], workscreen[ROWS][COLS] ;
char LIVE = '@' ;
char DEAD = '.' ;
unsigned int count ;
...
{
	FILE * ifp ;
	int 	die(void), clear_work(void), show_screen(void), 
		IS_START_FILE = 0 ,
...
/*----------
	loop logic.  This is the simplest method of
	programming this algorithm, so a beginner should
	grasp the logic in finite time.  It's certainly
	not the fastest way to do it.  I'm not real smart,
	so this approach was easy for me to understand.
----------*/
...
--------------------------------------------------*/
die() 	/*terminate & write a checkpoint file */
{
	int i , j, c ;
	FILE *ifp ;
